...
class SupabaseGateway:
    ...
    async def sett_status(self, request_id: str, status: str):
        logger.info("Updating status in Supabase", request_id=request_id, status=status)
        query = (
            f"UPDATE public.triage_results "
            f"SET status = {self._escape_sql_value(status)} "
            f"WHERE request_id = {self._escape_sql_value(request_id)} RETURNING id, status;"
        )
        try:
            result = await self.sql.execute_sql(query)
            logger.info("Status updated successfully", db_response=result)
            return result
        except Exception as e:
            logger.error("Failed to update status", error=str(e), exc_info=True)
            raise
